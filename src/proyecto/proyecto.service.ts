...
export class ProyectoService {
    ...
    async crearProyecto (proyecto: ProyectoEntity): Promise<ProyectoEntity> {
        if (proyecto.presupuesto>0 && proyecto.titulo.length>15)
            ...
            throw new BusinessLogicException("No se cumplen las precondiciones", BusinessError.PRECONDITION_FAILED)
}

     
async avanzarProyecto(id: number): Promise<ProyectoEntity> {
    const persistedProyecto = await this.proyectoRepository.findOne({where:{id}});

    if (!persistedProyecto){
        throw new BusinessLogicException("No se encontro ", BusinessError.NOT_FOUND)
    }
...
async findAll(proyecto: ProyectoEntity): Promise<EstudianteEntity> {
